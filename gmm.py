...
class GMM():
    ...
    def __init__(self,n_mixture,n_iter=100,print_every=1,crit=1e-5):
    ...
    def k_mean(self,data,K,rep=1,n_iter=1000,crit=1e-5):
        ...
        for k in range(0,K):
            if best_model[1][k].shape[0]<2+int(N/K/5):
                #retrain completely
                print('retrain k-mean because of a too small cluster')
                best_model=self.k_mean(data,K,rep=rep+1,n_iter=n_iter)
        return best_model

    def init_params(self,x):
        N=x.shape[0]
        [centroids,clusters,sum_distance]=self.k_mean(x,self.n_mixture,rep=1,n_iter=40)
        self.means=np.zeros((self.n_mixture,self.D))
    ...
    def sample(self,n):
        ...
        for i in range(0,n):
            sample[i,:]=np.random.multivariate_normal(self.means[components[i]],self.covars[components[i]],1)
        return sample
