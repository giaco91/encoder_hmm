...
def get_nonsequential_data(n,d=2,m=1):
	...
	for i in range(0,m-1):
		data[n_m*i:(i+1)*n_m,:]=np.random.multivariate_normal(centers[i,:],np.diag(np.ones(d)*np.power(m,-2/d))/sep,n_m)
	data[n_m*(m-1):,:]=np.random.multivariate_normal(centers[-1,:],np.diag(np.ones(d)*np.power(m,-2/d))/sep,n-n_m*(m-1))
